        ...
        {item && (
          <>
            ...
            <div className="portfolio-stage absolute inset-0">
              <div
                className={`portfolio-group absolute left-1/2 top-[70%] ${details ? "is-paused" : ""}`}
              >
                {/* Turntable floor */}
      ...
      {hover && item && !details && (
        <div
          ...
          style={{
            left: Math.min(
              hover.x + 18,
              (typeof window !== "undefined" ? window.innerWidth : 9999) - 270,
            ),
            top: hover.y + 18,
          }}
        >
          <div className="border border-chrome/40 bg-black/85 backdrop-blur-sm px-3 py-2">
            <div className="chrome-text text-base leading-tight">
              {item.title}
            </div>
...
function Standee({
  ...
  return (
    <button
      ...
      onClick={onClick}
      onPointerEnter={(e) =>
        e.pointerType === "mouse" && onHover?.(e.clientX, e.clientY)
      }
      onPointerMove={(e) =>
        e.pointerType === "mouse" && onHover?.(e.clientX, e.clientY)
      }
